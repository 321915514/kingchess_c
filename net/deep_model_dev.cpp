...
int DeepModel::InitModel(const std::string& path) {
    ...
    LOG(ERROR) << "Deep model loading: " << path;
    ...
    if (fin.fail()) {
        LOG(FATAL) << "Cannot read model file: " << path;
        return 0;
    }
    ...
    GetOutputs();
    LOG(ERROR) << ModelFlag::DebugString(m_branch_flag);

    m_context = std::unique_ptr<nvinfer1::IExecutionContext>(m_engine->createExecutionContext());
    if (!m_context) {
        LOG(FATAL) << "Cannot create context for model file: " << path;
        return 0;
    }
    LOG(ERROR) << "Deep model loaded: " << path;
    LOG(ERROR) << "Conv number = " << m_feature_handler.SquareNum();
    LOG(ERROR) << "Vector number = " << m_feature_handler.VectorNum();
    LOG(ERROR) << "Minibatch size = " << m_batch_size;
