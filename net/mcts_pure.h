...
#ifndef KINGCHESS_C_MCTS_PURE_H
#define KINGCHESS_C_MCTS_PURE_H
...
public:
    ...
    unsigned int select(std::shared_ptr<GameState> game,double c_puct);
    void expand(const std::unordered_map<int, float> &action_priors);
...
//         unsigned int num_mcts_sims, double c_virtual_loss); // deep_model *neural_network
    ...
    MCTS_Pure(unsigned int thread_num, double c_puct, unsigned int num_mcts_sims);
...
private:
    ...
    // variables
    std::unique_ptr<TreeNode, decltype(MCTS_Pure::tree_deleter) *> root;
