...
int run(){
    ...
    while (true){
       // print_board(game.board);
	    ...
	    auto res = mcts_resnet.get_action_probs(&game_copy,false,1e-3);
            mcts_resnet.update_with_move(-1);
	   ...
	   // alpha end
	    ...
	    // model
	    auto game_copy = GameState(game);
	    auto res = mcts_vit.get_action_probs(&game_copy,false,1e-3);
            mcts_vit.update_with_move(-1);
	    auto it = std::find(res.begin(),res.end(),1);
	    size_t index = std::distance(res.begin(),it);
...
int main() {
    ...
    std::cout<<"vit black:"<<black<<","<<" resnet white:"<<white<<std::endl;
}
